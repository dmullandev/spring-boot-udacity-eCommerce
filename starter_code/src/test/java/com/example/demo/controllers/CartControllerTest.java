...
public class CartControllerTest {
    ...
    public void addItemToCart() {
        ...
        assertEquals(itemTracker, response2.getBody().getItems());
        assertEquals(calculateCartTotalPrice(itemTracker), response2.getBody().getTotal());
    ...
    public void removeFromCart() {
        ...
        // one item, verify, then remove it
        when(mockItemRepository.findById(TEST_ITEM_ID)).thenReturn(
                Optional.of(buildTestItem(TEST_ITEM_ID, TEST_ITEM_NAME, TEST_ITEM_PRICE, TEST_ITEM_DESCRIPTION)));
        ...
        itemTracker.remove(buildTestItem(TEST_ITEM_ID, TEST_ITEM_NAME, TEST_ITEM_PRICE, TEST_ITEM_DESCRIPTION));

        ResponseEntity<Cart> response2 = testCartController.removeFromcart(buildModifyCartRequest(TEST_USER_USERNAME, TEST_ITEM_ID, 1));

        assertEquals(HttpStatus.OK, response2.getStatusCode());
        assertEquals(itemTracker, response2.getBody().getItems());
        assertTrue(response2.getBody().getItems().size() == 0);

        // user not found
        ResponseEntity<Cart> response3 = testCartController.removeFromcart(buildModifyCartRequest(TEST_USER_NOTFOUND, TEST_ITEM_ID2, 1));
        ...
        ResponseEntity<Cart> response4 = testCartController.removeFromcart(buildModifyCartRequest(TEST_USER_USERNAME, TEST_ITEM_ID3_NOTFOUND, 1));
